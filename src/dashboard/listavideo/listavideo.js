  ...
  const Lista = () => {
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          const response = await axios.get(`${APIHOST}/dadosvideo`);
    ...
    const enviarSolicitacao = (id_videos) => {
      axios.get(`${APIHOST}/apagarvideo/${id_videos}`)
        .then(response => {
        ...
        {noticiasExibidas.map((noticia) => (
          <div className="lista_conteudo" key={noticia.id}>
            <div className="coluna_conteudo_titulo">
              <p>{limitarCaracteres(noticia.titulo_video, 35)}</p>
            </div>
            ...
            <div className="coluna_conteudo_svg">
              ...
              <Link to={`/dashboard/editnoticia/${noticia.id_videos}`} className="link-noticia">
                ...
                <svg
                  ...
                  height="17"
                  viewBox="0 0 100 100"
                  onClick={() => enviarSolicitacao(noticia.id_videos)}
                >
